<template>
  <div class="container w-75 pb-5">
    <h1 class="mb-2 py-2 fw-bold text-white bg-danger bg-opacity-50 rounded-5">
      ACTOR DETAIL
    </h1>
    <h5 class="mb-4 py-3 fw-bold text-white bg-success bg-opacity-50 rounded-5">
      Please fill in the blanks.
      <strong style="color: greenyellow">Update</strong> or
      <strong style="color: rgb(253, 119, 119)">Delete</strong> an Actor.
    </h5>
    <div class="bg-body-secondary text-start py-3 px-5 rounded-5 shadow">
      <form @submit.prevent="handleUpdate()">
        ...
        <div class="text-center mb-2">
          <button class="btn btn-outline-success w-50 shadow">UPDATE</button>
        </div>
      </form>
      <div class="text-center">
        <button
          class="btn btn-outline-danger w-50 shadow"
          @click="handleDelete()"
        >
          DELETE
        </button>
    ...
    async handleUpdate() {
      ...
      if (this.currentActor.first_name === "") {
        ...
        const response = await updateActor(
          ...
          this.currentActor.first_name,
          this.currentActor.last_name
        );
        console.log(response);
      ...
      swalWithBootstrapButtons
        ...
        .then(async (result) => {
          if (result.isConfirmed) {
            const response = await deleteActor(this.$route.params.id);
            console.log(response);
